...
interface InboxListScreenProps {
  friends: IUserFriends[];
}

const InboxListScreen = ({ friends }: InboxListScreenProps) => {
  const [text, setText] = useState('');
  const [, setIsTyping] = useState(false);
  const [messages, setMessages] = useState([
    { type: 'their', content: '1' },
    { type: 'mine', content: '2' },
    { type: 'their', content: '3' }
    // ... (các tin nhắn khác)
  ]);
  console.log(friends);
  ...
  const sendMsg = () => {
    friends.forEach(friend => {
      ...
      const messagesRef = database().ref(`/messagess`).push();

      // Lấy key được tạo tự động
      const autoGeneratedKey = messagesRef.key;
      console.log('Auto generated key: ', autoGeneratedKey);
